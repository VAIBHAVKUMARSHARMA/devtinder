...
import { Send, AlertCircle, ArrowLeft, Users, Sparkles, Loader2 } from "lucide-react";
...
const ChatPage = () => {
  ...
  const handleGenerateIcebreaker = async () => {
    ...
    try {
      ...
      const data = await getIcebreaker(activeChat, context);
      if (data && data.icebreaker) {
        setMessage(data.icebreaker);
      }
    } catch (error) {
      console.error("Failed to generate icebreaker:", error);
      setIcebreakerError("Failed to generate icebreaker. Please try again.");
      setTimeout(() => setIcebreakerError(null), 3000);
    } finally {
                ...
                {connections &&
                  ...
                  connections.map((connection) => {
                    ...
                    return (
                      <div
                        key={connection._id}
                        className={`flex items-center space-x-4 p-2 rounded-lg cursor-pointer transition-all duration-200 hover:scale-[1.02] ${activeChat === connection._id
                          ? "bg-primary/10"
                          : "hover:bg-accent"
                          }`}
                        ...
                        {messages && activeChat && messages[activeChat] ? (
                          messages[activeChat].map((msg, index) => {
                            ...
                            return (
                              ...
                              >
                                <div
                                  className={`max-w-[70%] rounded-lg p-3 ${isSentByMe
                ...
                <div className="p-2 sm:p-4 border-t flex-shrink-0">
                  <form onSubmit={handleSendMessage} className="flex space-x-2 items-center">
                    <Button
                      ...
                      disabled={isGeneratingIcebreaker || !activeChat}
